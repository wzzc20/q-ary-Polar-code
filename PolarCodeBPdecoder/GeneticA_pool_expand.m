function [sampleM_expand] = GeneticA_pool_expand(N, K, ebno1,ebno2,M,GA_itermax,alpha,beta)
...
%generate M samples
    ...
    ber_expand = zeros(1,M+GA_itermax);
    sampleM_expand = zeros(M+GA_itermax,N);
    ber = ber1.*ber2;
    [~,order] = sort(ber);
    ber_expand(1:M) = ber;
    sampleM = sampleM(order,:);
    sampleM_expand(1:M,:) = sampleM;
    ...
    for iter = 1:GA_itermax
        if(mod(iter,100)==0)
            disp(['Sim iteration running = ', num2str(iter)]);
        end
        pdf = 1:M+iter-1;
        pdf = exp(-alpha*pdf);
        pdf = pdf/sum(pdf);
        ...
        [~,p1] = histc(rand,cdf);
        [~,p2] = histc(rand,cdf);
        sample1 = sampleM_expand(p1,:);
        sample2 = sampleM_expand(p2,:);
        ...
        ber_expand_1 = [ber_expand(1:M+iter-1),ber_merge_selectk];
        sampleM_expand_1 = [sampleM_expand(1:M+iter-1,:);merge_selectk];
        
        [ber_expand_1,order] = sort(ber_expand_1);
        sampleM_expand_1 = sampleM_expand_1(order,:);
        ber_expand(1:M+iter) = ber_expand_1;
        sampleM_expand(1:M+iter,:) = sampleM_expand_1;
    end
    toc
    %disp(sampleM(1:5,:));
end
