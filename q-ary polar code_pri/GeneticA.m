function [sampleM] = GeneticA(N, K,q, ebno1,ebno2,M,GA_itermax,alpha_decay,beta,alpha,use_prior,prior)
...
%generate M samples
    ...
    for i = 1:M
        ...
        [~, ber1(i)] = Simulation_given_construction(max_iter, max_err, max_runs, find(sampleM(i,:)==1), ebno1, N, K,q,addchart,mulchart,divchart,alpha);
        [~, ber2(i)] = Simulation_given_construction(max_iter, max_err, max_runs, find(sampleM(i,:)==1), ebno2, N, K,q,addchart,mulchart,divchart,alpha);
    end
    toc
    ber = ber1.*ber2;
    [ber,order] = sort(ber);
    ...
    for iter = 1:GA_itermax
        ...
        [~, ber1] = Simulation_given_construction(max_iter, max_err, max_runs, find(merge_selectk==1), ebno1, N, K,q,addchart,mulchart,divchart,alpha);
        [~, ber2] = Simulation_given_construction(max_iter, max_err, max_runs, find(merge_selectk==1), ebno2, N, K,q,addchart,mulchart,divchart,alpha);
