function x = polar_encoder(u,q, lambda_offset, llr_layer_vec,addchart,mulchart,divchart,alpha)
%encoding: x = u * Fn.
N = length(u);
m = log2(N);
C = zeros(N - 1, 1);
...
for phi = 0 : N - 1
    switch phi
        case 0
            ...
            for beta = 1 : index_1
                C(beta + index_1 - 1) = addchart(u(beta)+1,mulchart(u(beta + index_1)+1,alpha+1)+1);
            end
            for i_layer = m - 2 : -1 : 0
                index_1 = lambda_offset(i_layer + 1);
                index_2 = lambda_offset(i_layer + 2);
                for beta = index_1 : index_2 - 1
                    C(beta) = addchart(C(beta + index_1)+1 ,mulchart(C(beta + index_2)+1,alpha+1)+1);
                end
            end
        case N/2
            index_1 = lambda_offset(m);
